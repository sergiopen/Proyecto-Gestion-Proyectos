...
if(isset($_COOKIE['sesion'])) {
    $datos_sesion = json_decode(stripslashes($_COOKIE['sesion']), true);
    $rol = $datos_sesion['rol'];
    $esAdmin = $rol === 'admin';
}

if ($esAdmin && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['eliminar']) && isset($_POST['id_profesor'])) {
...
if ($esAdmin && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['eliminar_multiple']) && !empty($_POST['profesores_seleccionados'])) {
...
echo '<input type="text" name="nombre" id="nombre" placeholder="Nombre del profesor" value="' . esc_attr($nombre_filtro) . '">';
echo '<label for="email">Email:</label>';
echo '<input type="text" name="email" id="email" placeholder="Email del profesor" value="' . esc_attr($email_filtro) . '">';
...
if ($resultado->num_rows > 0) {
    if ($esAdmin) echo '<form method="POST" onsubmit="return confirm(\'¿Estás seguro de que quieres eliminar los profesores seleccionados?\');">';
    echo '<table border="1" class="tabla tabla-profesores">';
    echo '<thead><tr>';
    if ($esAdmin) echo '<th><input type="checkbox" id="checkTodos"></th>';
    echo '<th>ID</th><th>Nombre de usuario</th><th>Nombre</th><th>Apellido</th><th>Email</th><th>Rol</th>';
    if ($esAdmin) echo '<th>Editar</th><th>Eliminar</th>';
    echo '</tr></thead>';
    echo '<tbody>';

    while ($row = $resultado->fetch_assoc()) {
        echo '<tr>';
        if ($esAdmin) echo '<td><input type="checkbox" name="profesores_seleccionados[]" value="' . esc_html($row['id']) . '"></td>';
    ...
    echo '</tbody>';
    echo '</table>';
    if ($esAdmin) echo '<button type="submit" name="eliminar_multiple" class="btn-filtrar btn-eliminar-multiple">Eliminar seleccionados</button>';
    if ($esAdmin) echo '</form>';
} else {
